...
export default {
  ...
  // 获取token
  getToken: (data) => request.post('/oidc-server/oauth2/token', data, { noNeedToken: true, noNeedTip: true ,headers:{
      Authorization: `Basic dnVlQ2xpZW50OnZ1ZUNsaWVudA==`
    }}),
  // 刷新token
  refreshToken: (data) => request.post('/oidc-server/oauth2/token', data, { noNeedToken: true, noNeedTip: false ,headers:{
      Authorization: `Basic dnVlQ2xpZW50OnZ1ZUNsaWVudA==`
    }}),
  // 登出
