...
import { getConsentParameters, submitApproveScope } from '@/views/oauth2/login/index.js'

const { message } = createDiscreteApi(['message'])

// 获取授权确认信息响应
const consentResult = ref()
// 所有的scope
const scopes = ref()
// 已授权的scope
...
getConsentParameters(window.location.search)
  .then((result) => {
    if (result.success) {
      ...
      approvedScopes.value = result.data.previouslyApprovedScopes.map(e => e.scope)
    }
    else {
...
function submitApprove(cancel) {
  ...
  if (!cancel) {
    ...
    if (
      ...
      approvedScopes.value.forEach(e => data.append('scope', e))
    }
  }
...
<template>
  ...
  <main>
    ...
    <br>
    <n-card v-if="consentResult" :title="`${consentResult.clientName} 客户端`">
    ...
    <br>
    <n-button type="info" :loading="loading" strong @click="submitApprove(false)">
      &nbsp;&nbsp;&nbsp;&nbsp;确&nbsp;&nbsp;&nbsp;&nbsp;定&nbsp;&nbsp;&nbsp;&nbsp;
    </n-button>
    &nbsp;&nbsp;&nbsp;&nbsp;
