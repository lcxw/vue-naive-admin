...
import { base64Str } from '@/utils/oauth2/pkce/index.js'
 ...
 * @returns 登录状态
 */
export function getSmsCaptchaByPhone(params) {
  return loginRequest.get({
    url: '/getSmsCaptcha',
    params,
  })
 ...
 * @returns 授权确认页面相关数据
 */
export function getConsentParameters(queryString) {
  return loginRequest.get({
    // url: `/api/oidc-server/oauth2/consent/parameters${queryString}`,
 ...
 * @returns 是否确认成功
 */
export function submitApproveScope(data, requestUrl) {
  return loginRequest.post({
    url: requestUrl,
    data,
    headers: {
...
export function deviceVerification(data) {
  return loginRequest.post({
    // url: `/api/oidc-server/oauth2/device_verification`,
