...
import { getImageCaptcha, getSmsCaptchaByPhone, loginSubmit } from '@/views/oauth2/login/index.js'
import { router } from '@/router/index'
import { getQueryString } from '@/utils/oauth2/GlobalUtils'
import { fetch, generateQrCode } from '@/views/oauth2/login/QrCodeLogin'
    ...
    <n-card title="">
      <n-tabs default-value="signin" size="large" justify-content="space-evenly" @update:value="handleUpdateValue">
        <n-tab-pane name="signin" tab="账号登录">
          <n-form>
            ...
            <n-form-item-row label="密码">
              <n-input
                v-model:value="loginModel.password" type="password" show-password-on="mousedown"
                placeholder="密码"
              />
            </n-form-item-row>
          ...
          <n-button type="info" :loading="loading" strong block @click="submitLogin('passwordLogin')">
            登录
          </n-button>
        </n-tab-pane>
        <n-tab-pane name="signup" tab="短信登录">
          <n-form>
            ...
            <n-form-item-row label="验证码">
              <n-input-group>
                ...
                <n-button type="info" style="width: 130px" :disabled="counterActive" @click="getSmsCaptcha">
                  获取验证码
                  <span v-if="counterActive">
                    (
                    <n-countdown
                      :render="renderCountdown" :on-finish="onFinish" :duration="59 * 1000"
          ...
          <n-button type="info" :loading="loading" strong block @click="submitLogin('smsCaptcha')">
            登录
          </n-button>
        </n-tab-pane>
      ...
      <div v-if="showThirdLogin" class="other_login_icon">
        <IconGitee :size="32" class="icon_item" @click="thirdLogin('gitee')" />
        <img width="36" height="36" src="../../assets/GitHub-Mark.png" class="icon_item" @click="thirdLogin('github')">
        <img width="28" height="28" src="../../assets/wechat_login.png" class="icon_item" @click="thirdLogin('wechat')">
      </div>
    </n-card>
  </main>
</template>
